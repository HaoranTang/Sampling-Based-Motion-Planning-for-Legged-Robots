...
function [bool_success,val] = fcn_steer_init(vout)
% bool_success
...
for istep = 1:Nstep
    ...
    F = [F;(dzi == 0):'dzi(x)'];
    F = [F;(dxf == vout(1)):'dxf'];
    F = [F;(dzf == vout(2) - g*(xf/vout(1))):'dzf(x)'];
end

Fquad = cst_quadratic_approx(xf,xf2,12,[xmin;xmax]);
F = [F;Fquad:'xf^2 = xf2'];

% Fquad = cst_quadratic_approx(xi,xi2,7,[xmin;xmax]);
% F = [F;Fquad:'xi^2 = xi2'];
...
% obj = -qTD(1,end);
norm_v = norm([dxi;dzi]);
normi = norm(zf + (g/(2*vout(1)^2))*xf2 - (vout(2)/vout(1)) * xf);
obj = norm_v + normi;
...
if sol.problem == 0
    ...
    val.Fx_var = value(Fx_var);
    val.Fz_var = value(Fz_var);
    val.Tsolve = sol.solvertime;
...
p.Nr = 3;                     % number of nodes in r direction
p.Nth = 5;                    % number of nodes in th direction
p.M_st = 7;                   % number of sampling points in stance phase
